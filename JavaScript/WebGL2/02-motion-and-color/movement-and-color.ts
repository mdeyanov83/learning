...
const CIRCLE_SEGMENTS_COUNT = 30;
...
void main() {
 ...
 function buildCircleVertexBufferData() {
    ...
    for (let i = 0; i < CIRCLE_SEGMENTS_COUNT; i++) {
        const vertex1Angle = i * Math.PI * 2 / CIRCLE_SEGMENTS_COUNT;
        const vertex2Angle = (i+1) * Math.PI * 2 / CIRCLE_SEGMENTS_COUNT;
        ...
        // Center vertex is a light blue collor and in the middle of the shape
        vertexData.push(
            // Position (x, y)
            0, 0,
            // Color (r, g, b)
        ...
        // Other two vertices are along the edges of the circle, and are darker blue color
        vertexData.push(
            // Position (x, y)
            x1, y1,
            // Color (r, g, b)
...
function movementAndColorDemo() {
    ...
    const graySquareVao = createTwoBufferVao(
        gl, squareGeoBuffer, graySquareColorBuffer,
        vertexPositionAttributeLocation, vertexColorAttributeLocation);
    const circleVao = createInterleavedBufferVao(
        gl, circleInterleavedBuffer,
        vertexPositionAttributeLocation, vertexColorAttributeLocation);
    ...
    const geometryList = [
        ...
        { vao: circleVao, numVertices: CIRCLE_SEGMENTS_COUNT * 3 }
    ]
    ...
    const frame = function () {
        ...
        gl.useProgram(movementAndColorProgram); // GPU program (vertex + fragment shader pair)
